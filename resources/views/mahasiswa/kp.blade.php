<x-app-layout>
    <div x-data="">
                        ...
                        <li class="mb-10 ms-6 flex items-center">
                            @if(isset($kp->metadata))
                                @if($kp->metadata->status == 'awaited')
                                    ...
                                    <div class="">
                                        <h3 class="font-medium leading-tight">Data KP</h3>
                                        <p class="text-sm">Data KP anda sedang diulas</p>
                                    </div>
                                @elseif($kp->metadata->status == 'reviewed')
                                    ...
                                    <div class="">
                                        <h3 class="font-medium leading-tight">Data KP</h3>
                                        <p class="text-sm">Data KP anda ditolak dan perlu diperbaiki</p>
                                    </div>
                                @elseif($kp->metadata->status == 'done')
                                    ...
                                    <div class="">
                                        <h3 class="font-medium leading-tight">Data KP</h3>
                                        <p class="text-sm">Data KP anda sudah disetujui</p>
                                    </div>
                                @endif
                        ...
                        <li class="mb-10 ms-6 flex items-center">
                            ...
                            @else
                                @if($suratIzin->status == 'awaited')
                                    ...
                                    <div class="">
                                        <h3 class="font-medium leading-tight">Surat Izin</h3>
                                        <p class="text-sm">Surat Izin KP anda sedang diulas</p>
                                    </div>
                                @elseif($suratIzin->status == 'reviewed')
                                    <span class="absolute flex items-center justify-center w-8 h-8 bg-green-200 rounded-full -start-4 ring-4 ring-white dark:ring-gray-900 dark:bg-yellow-800">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5 text-green-500 dark:text-yellow-400">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
                                        </svg>
                                    ...
                                    <div class="">
                                        <h3 class="font-medium leading-tight">Surat Izin</h3>
                                        <p class="text-sm">Surat Izin KP anda ditolak dan perlu diperbaiki</p>
                                    </div>
                                @elseif($suratIzin->status == 'done')
                                    <span class="absolute flex items-center justify-center w-8 h-8 bg-green-200 rounded-full -start-4 ring-4 ring-white dark:ring-gray-900 dark:bg-green-900">
                                        <svg class="w-3.5 h-3.5 text-green-500 dark:text-green-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 12">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5.917 5.724 10.5 15 1.5"/>
                                        </svg>
                                    ...
                                    <div class="">
                                        <h3 class="font-medium leading-tight">Surat Izin</h3>
                                        <p class="text-sm">Surat Izin KP anda sudah disetujui</p>
                                    </div>
                                @endif
                            @endif
                        </li>
            ...
            <div class="w-full mx-auto sm:px-6 lg:px-8 mt-4">
                ...
                <div class="w-full mx-auto">
                    <div class="bg-white dark:text-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg px-4 py-4 h-full">
                        Form Data KP
                        <form action="{{ route('mahasiswa.kp.metadata') }}" method="POST" @submit.prevent="uploadFile">
                            @csrf
                            @method('PATCH')
                            <input type="hidden" name="status" value="{{$kp->metadata ? $kp->metadata->status : ''}}" >
                            <!-- input Judul KP -->
                            <x-form-text
                                label="Judul KP" 
                                name="judul" 
                            ...
                            @elseif($kp->metadata->status == 'awaited' || $kp->metadata->status == 'reviewed')
                                <div class="flex {{$kp->metadata->status == 'reviewed' ? 'justify-between' : 'justify-end'}} mt-2">
                                    @if($kp->metadata->status == 'reviewed')
                                        <x-button tag="button" type="button" color="warning"
                                            x-on:click.prevent="$dispatch('open-modal', 'revisiJudul')"
                                        >
                                            Lihat Pesan Revisi
                                        </x-button>
                                    @endif
                ...
                <div x-data="{ suratIzinFile: '{{ $suratIzinFile ?? '' }}' }" @dragover.prevent @dragenter.prevent @drop.prevent="suratIzinFile = $event.dataTransfer.files[0].name">
                    <form action="{{ route('mahasiswa.kp.suratIzinPost') }}" method="POST" enctype="multipart/form-data" @submit.prevent="uploadFile($event)">
                        ...
                        <div class="bg-white my-3 dark:text-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg px-4 py-4">
                            ...
                            @if($kp->suratizin?->status == 'awaited' || $kp->suratIzin?->status == 'reviewed' || !isset($kp->suratizin))
                            <div class="flex justify-between mt-2" x-show="suratIzinFile">
                                ...
                                <x-button tag="button" color="danger" type="button" @click.prevent="suratIzinFile = ''" x-show="suratIzinFile != '{{$suratIzin ? $suratIzin->file_name : ''}}' || '{{ $suratIzin ? $suratIzin->status : '' }}' == 'reviewed'">

                                    {{ isset($suratIzinFile) ? 'replace' : 'Cancel'}}
                                </x-button>
                                @if($kp->surat_izin && $kp->surat_izin->status == 'reviewed')
                                <x-button tag="button" type="button" color="warning"
                                    x-on:click.prevent="$dispatch('open-modal', 'revisiSuratIzin')"
                                >
                                    Lihat Pesan Revisi
                                </x-button>
                                @endif
        ...
        {{-- modal pesan revisi judul --}}
        <x-modal name="revisiJudul" focusable>
            <div class="p-6">
                <div class="flex items-center justify-between p-2 text-lg font-bold dark:text-white">
                    <span>
                        Pesan dari kordinator untuk perbaikan judul
                    </span>
                </div> 
                <x-textarea
        ...
        {{-- end modal revisi judul --}}
        {{-- modal pesan revisi surat izin --}}
        <x-modal name="revisiSuratIzin" focusable>
            <div class="p-6">
                <div class="flex items-center justify-between p-2 text-lg font-bold dark:text-white">
                    <span>
                        Pesan dari kordinator untuk perbaikan judul
